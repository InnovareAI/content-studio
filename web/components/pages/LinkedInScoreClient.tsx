...
const PROFILE_URL = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/linkedin-profile-score`
const BREW_URL    = `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/brew360-audit`
...
function AuditContextCard({ orgId, projectId, accessToken }: { orgId: string; projectId: string | null; accessToken: string | null }) {
  ...
  async function extractFromWebsite() {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/extract-audit-intent`, {
        method: 'POST',
