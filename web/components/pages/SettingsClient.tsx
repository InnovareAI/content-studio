...
function TeamTab() {
  ...
  async function sendInvite() {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/client-invites`, {
        method: 'POST',
...
function AiUsageTab() {
  ...
  const loadPricingCatalog = useCallback(async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const res = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/model-pricing-admin`, {
        method: 'POST',
        headers: {
